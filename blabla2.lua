...
RunService.Heartbeat:Connect(function()
    ...
    if CounterFollow then
        ...
        if target and target:FindFirstChild("HumanoidRootPart") then
            ...
            -- Teleport depan target jarak 3, lock posisi
    ...
    elseif PurpleFollow then
        ...
        if target and target:FindFirstChild("HumanoidRootPart") then
            ...
            hrp.CFrame = CFrame.new(thrp.Position + Vector3.new(-15, 25, 0), thrp.Position)
            hrp.Velocity = Vector3.zero 
            hrp.AssemblyLinearVelocity = Vector3.zero
        end
...
task.spawn(function()
    while true do
        ...
        CurrentCounterTarget = GetValidTargetFromList(PurpleTargetList)
        if not CurrentPurpleTarget then
            CounterFollow = false
            PurpleFollow = false
            ForceKillByVoid() 
            repeat 
                task.wait(1)
                CurrentPurpleTarget = GetValidTargetFromList(PurpleTargetList)
            until not AutoGojoRework or CurrentPurpleTarget
            task.wait(10)
            continue
        end
