...
local Window = Rayfield:CreateWindow({
    Name = "Auto Gojo + Auto Loot (Reworked)",
    ...
    if BlacklistDropdown then
        ...
        BlacklistDropdown = LootTab:CreateDropdown({
            ...
            Callback = function(selected)
                ...
                for _, name in ipairs(selected) do ItemBlacklist[name] = true end
            end
        })
    end
end
...
local function GetTeleportCFrame(targetHRP)
    return CFrame.new(targetHRP.Position + targetHRP.CFrame.LookVector * TeleportDistance, targetHRP.Position)
end
...
RunService.Heartbeat:Connect(function()
    ...
    if HollowFollow then hrp.CFrame = CFrame.new(thrp.Position + Vector3.new(-15, 26, 0), thrp.Position)
    elseif RedFollow then hrp.CFrame = CFrame.new(thrp.Position + Vector3.new(0, 26, 0), thrp.Position)
    elseif FollowTarget then hrp.CFrame = GetTeleportCFrame(thrp) end
end)

-- Mastery/Breakthrough Loop
task.spawn(function()
    while task.wait(1) do
        ...
        if data and exp and globalRemotes and exp.Value >= 30725 then
            if AutoMastery then pcall(function() globalRemotes:FindFirstChild("UpgradeMas"):FireServer() end) end
            if AutoBreakthrough then pcall(function() globalRemotes:FindFirstChild("Breakthrough"):FireServer() end) end
        end
    end
end)
...
task.spawn(function()
    while task.wait(1) do
        if AutoClear then
            pcall(function()
                local args = {buffer.fromstring("\018"), buffer.fromstring("\254\001\000\006\005Clear")}
...
task.spawn(function()
    while task.wait(0.5) do
        if not AutoLoot or FollowTarget or HollowFollow or RedFollow or AutoGojoRework then 
            LootingActive = false
            continue 
        end
        ...
        if #prompts > 0 then
            ...
            for _, prompt in ipairs(prompts) do
                if not AutoLoot or FollowTarget or HollowFollow or RedFollow or AutoGojoRework then break end
...
task.spawn(function()
    while true do
        task.wait(0.5)
        if not AutoRed then RedFollow = false continue end
        CurrentRedTarget = GetValidTargetFromList(RedTargetList)
        if not CurrentRedTarget then
            RedFollow = false
            ForceKillByVoid()
            repeat task.wait(1) CurrentRedTarget = GetValidTargetFromList(RedTargetList) until not AutoRed or CurrentRedTarget
            continue
        end
        UseFold(0.5)
        RedFollow = true
        task.wait(0.1)
        pcall(function() Gojo.RevRed2:FireServer() end)
        task.wait(3)
        RedFollow = false
        ForceKillByVoid()
    end
...
task.spawn(function()
    while true do
        ...
        if not CurrentGojoTarget then
            ForceKillByVoid()
            repeat task.wait(1) CurrentGojoTarget = GetValidTargetFromList(GojoTargetList) until not AutoGojo or CurrentGojoTarget
            continue
        end
        UseFold(0.5)
        FollowTarget = true
        ...
        while AutoGojo and LocalPlayer.Character.Humanoid.Health > 54 do task.wait() end
        FollowTarget = false
        CreateSafeZone()
        LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(0, SAFE_Y + 5, 0)
        UseHeal(1.85)
        UseResurrect()
        task.wait(2.5)
        FollowTarget = true
        ...
        HollowFollow = true
        task.wait(52)
        pcall(function() Gojo.HollowPurple:FireServer() end)
        task.wait(23)
        HollowFollow = false
        ForceKillByVoid()
    end
...
task.spawn(function()
    while true do
        ...
        local targetName = "Counter Dummy"
        ...
        -- STEP 1: Memukul sampai HP rendah
        ...
        -- STEP 3: Memukul selama 10 Detik
        local reEngageStart = os.clock()
        local nextPunchStep3 = 0
        while (os.clock() - reEngageStart < 10) and AutoGojoRework do
        ...
        -- STEP 4: Persiapan Hollow Purple (Terbang di atas)
        task.wait(10)
        ...
        if AutoGojoRework then pcall(function() Gojo.HollowPurple:FireServer() end) end
        task.wait(23)
        if AutoGojoRework then ForceKillByVoid() end
    end
end)