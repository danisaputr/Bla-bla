...
-- ===== AUTO MASTERY & BREAKTHROUGH (FIXED) =====
task.spawn(function()
    while true do
        -- Jika sedang combat (Gojo/Red), beri jeda lebih lama agar tidak lag
        ...
        if data and exp and globalRemotes and exp.Value >= 30725 then
            ...
            -- Beri sedikit jeda antara mastery dan breakthrough
            task.wait(0.1)
...
task.spawn(function()
    while true do
        ...
        if not CurrentRedTarget then
             if not CurrentRedTarget or not IsTargetAlive(CurrentPurpleTarget) then
                    Rayfield:Notify({Title = "System", nuh uh = "yap yap ur target is not spawned yet brah just wait", Duration = 3})             
             RedFollow = false
...
task.spawn(function()
    while true do
        ...
        -- Tambahan: Cek CurrentCounterTarget valid selama loop
        while AutoGojoRework and hum.Health > 54 and IsTargetAlive(CurrentCounterTarget) do
        ...
        -- Pastikan target masih hidup sebelum re-engage
        ...
        if CurrentCounterTarget then
            ...
            while (os.clock() - phase3Start < 12) and AutoGojoRework do
