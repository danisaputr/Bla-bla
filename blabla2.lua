...
UtilityTab:CreateSection("Auto Clear Filtered (Exp)")
local WhitelistDropdown = UtilityTab:CreateDropdown({
    Name = "Whitelist Items (YANG DISIMPAN)",
    Options = getItemNameList(), -- Mengambil list nama item dari fungsi yang sudah ada
    CurrentOption = {},
    MultipleOptions = true,
    Multi = true,
    Flag = "ExpWhitelist", 
    Callback = function(opts)
        ExpWhitelist = opts
        -- Konversi tabel ke format key-value agar pengecekan lebih cepat (opsional tapi bagus)
    end
})
...
UtilityTab:CreateToggle({
    ...
    Callback = function(v)
        AutoClearExp = v
        if v then
            Rayfield:Notify({Title = "System", Content = "Auto Clear Exp ON: Menghapus item selain Whitelist!", Duration = 3})
        end
    end
})
                ...
                if backpack then
                    for _, item in ipairs(backpack:GetChildren()) do
                        if item:IsA("Tool") then
                            ...
                            -- Cek apakah nama item ada di dalam ExpWhitelist
                            ...
                            -- Jika item TIDAK ada di list aman, maka HAPUS/DESTROY
                            if not isSafe then
                                item:Destroy()
                                -- Catatan: Jika game memiliki remote khusus untuk 'Drop' atau 'Trash', 
                                -- ganti item:Destroy() dengan remote tersebut. 
                                -- Tapi biasanya item:Destroy() bekerja untuk membersihkan backpack client.
                            end
                        end
                    end
                end
