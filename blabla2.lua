...
local Window = Rayfield:CreateWindow({
    Name = "Auto Gojo (Rework Follow Fixed)",
    LoadingTitle = "System Loaded",
    LoadingSubtitle = "Strict Dummy Targeting",
...
Tab:CreateToggle({
    Name = "Auto Gojo REWORK (Target: Counter Dummy)",
    CurrentValue = false,
    Callback = function(v)
        AutoGojoRework = v
        if v then
...
-- =======================================================
-- ===== UNIFIED FOLLOW SYSTEM (FIXED TARGETING) =========
-- =======================================================
RunService.Heartbeat:Connect(function()
    ...
    -- === 1. TARGET SELECTION (STRICT MODE FOR REWORK) ===
    local activeTarget = nil

    if AutoGojoRework then
        -- FIXED: Strict targeting. HANYA Counter Dummy.
        -- Jangan fallback ke CurrentGojoTarget (Player) untuk menghindari salah TP.
        if Workspace:FindFirstChild("Living") then
            activeTarget = Workspace.Living:FindFirstChild("Counter Dummy")
        end
    elseif AutoGojo then
        activeTarget = CurrentGojoTarget
    elseif AutoRed then
        activeTarget = CurrentRedTarget
    end
    ...
    -- === 2. TELEPORT LOGIC BERDASARKAN STATE ===
    if CounterFollow then
        -- Logic Close Range (Punch)
        hrp.CFrame = CFrame.new(thrp.Position + Vector3.new(0, 0, -3), thrp.Position)
        hrp.Velocity = Vector3.zero
        hrp.AssemblyLinearVelocity = Vector3.zero

    elseif HollowFollow or RedFollow then
        -- Logic Long Range (Hollow Purple / Red)
        -- FIXED: Ini sekarang akan mengunci ke Dummy jika mode Rework aktif
        hrp.CFrame = CFrame.new(thrp.Position + Vector3.new(0, 26, 0), thrp.Position)
        hrp.Velocity = Vector3.zero 
        hrp.AssemblyLinearVelocity = Vector3.zero

    elseif FollowTarget then
        -- Logic Combat Player Biasa
...
task.spawn(function()
    while task.wait(0.5) do
        ...
        if FollowTarget or HollowFollow or RedFollow or CounterFollow then
            LootingActive = false
            continue
        end
        ...
        if #prompts > 0 then
            ...
            for _, prompt in ipairs(prompts) do
                ...
                if FollowTarget or HollowFollow or RedFollow or CounterFollow then break end
...
-- ==== AUTO GOJO REWORK LOOP (COUNTER DUMMY) ====
task.spawn(function()
    while true do
        task.wait(0.2)
        ...
        -- FIXED: Loop ini sekarang mencari Counter Dummy secara eksplisit
        local dummyTarget = nil
        if Workspace:FindFirstChild("Living") then
            dummyTarget = Workspace.Living:FindFirstChild("Counter Dummy")
        end

        if not dummyTarget or not dummyTarget:FindFirstChild("HumanoidRootPart") then
            CounterFollow = false
            HollowFollow = false
            -- Jika dummy tidak ada, tunggu atau bunuh diri agar reset
            ForceKillByVoid() 
            ...
            until not AutoGojoRework or (dummyTarget and dummyTarget:FindFirstChild("HumanoidRootPart"))
        ...
        -- Aktifkan Follow untuk Punch (Close Range)
        CounterFollow = true
        task.wait(0.5)
        ...
        -- Hollow Purple Phase
        CounterFollow = false
        HollowFollow = true   -- Ini akan memicu TP ke Safezone Udara di atas Dummy
