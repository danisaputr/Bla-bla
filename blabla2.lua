...
local IsSavingToken = false -- [UPDATED] Variable ini dipindah ke Global agar bisa dibaca Auto Clear
...
-- ===== AUTO CLEAR LOOP (UPDATED) =====
task.spawn(function()
    while true do
        -- Logic: Jika BBQ3 Nyala DAN BUKAN Mode Saving Token (artinya sedang farming biasa/crafting/summon)
        -- Maka Pause Auto Clear agar item tidak terhapus.
        local ShouldPauseClear = AutoBBQ3 and not IsSavingToken

        if AutoClear and not ShouldPauseClear then
            pcall(function()
                local args = {
            ...
            -- Jika dipause atau tidak aktif, tunggu sebentar sebelum cek lagi
            task.wait(1)
        end
    end
end)
...
-- ===== [NEW] AUTO BBQ3 SUMMON LOGIC (WITH COMBAT PAUSE & CLEAR DELAY) =====
task.spawn(function()
    -- [NOTE] Variable IsSavingToken sudah dipindah ke Global (atas)
    ...
    while true do
        ...
        if ultraCount == 0 and meatCount < 5 then
            ...
            if IsSavingToken then
                if tokenValue >= 5000 then
                    IsSavingToken = false -- Target tercapai, lanjut eksekusi. Auto Clear akan PAUSE lagi.
                    ...
                    -- Saat di sini, IsSavingToken == true, jadi Auto Clear diizinkan jalan
            ...
            elseif tokenValue < 1000 then
                IsSavingToken = true -- Auto Clear akan diizinkan jalan
                IsSummoningAction = false -- PENTING: Combat ON supaya bisa farming token
        ...
        -- Di fase ini, kita PAUSE COMBAT (IsSummoningAction = true)
        -- Dan karena IsSavingToken = false, maka Auto Clear akan PAUSE (agar item tidak hilang)
        IsSummoningAction = true
