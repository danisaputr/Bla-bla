...
local CounterFollow = false -- State variable untuk Counter Dummy
...
local Window = Rayfield:CreateWindow({
    Name = "Auto Gojo (Counter Follow Fixed)",
    LoadingTitle = "System Loaded",
    LoadingSubtitle = "Tracking Optimized",
...
RunService.Heartbeat:Connect(function()
    ...
    if AutoGojoRework then
        -- Rework: Cari Counter Dummy dulu
        activeTarget = Workspace.Living:FindFirstChild("Counter Dummy")
        -- Jika dummy ga ada, fallback ke player target
        if not activeTarget then activeTarget = CurrentGojoTarget end
    ...
    while true do
        ...
        if not CurrentGojoTarget then
            ...
            repeat 
                task.wait(1) 
                CurrentGojoTarget = GetValidTargetFromList(GojoTargetList)
            until not AutoGojoRework or CurrentGojoTarget
