...
local Window = Rayfield:CreateWindow({
    Name = "Auto Gojo + Auto Loot (Teleport Fixed)",
    LoadingTitle = "System Loaded",
    LoadingSubtitle = "Hollow TP Updated",
...
-- ===== FOLLOW SYSTEM (HOLLOW SYNCED WITH RED) =====
RunService.Heartbeat:Connect(function()
    ...
    -- === LOGIKA KHUSUS HOLLOW FOLLOW (SAMA SEPERTI RED) ===
    if HollowFollow then
        if CurrentGojoTarget and CurrentGojoTarget:FindFirstChild("HumanoidRootPart") then
            ...
            -- Teleport di atas kepala target (Synced with Red Logic)
            hrp.CFrame = CFrame.new(thrp.Position + Vector3.new(0, 26, 0), thrp.Position)
        end
        return 
    end

    -- === LOGIKA FOLLOW LAINNYA ===
...
task.spawn(function()
    while true do
        ...
        FollowTarget = false
        HollowFollow = true -- Memicu teleport ke CurrentGojoTarget (Player)
