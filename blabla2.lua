...
local Window = Rayfield:CreateWindow({
    ...
    LoadingSubtitle = "Configuration Active",
    ConfigurationSaving = {
        Enabled = true,
        FolderName = "GojoReworkConfig", -- Folder khusus untuk config ini
        FileName = "Settings"
    }
})
...
-- [FIX] Added Flag and fixed string syntax
local CounterDropdown = Tab:CreateDropdown({
    Name = "[Rework] Counter Targets (Punch Phase)",
    Options = GetTargets(),
    CurrentOption = {"Counter Dummy"}, -- FIX: Harus String dalam Table
...
Tab:CreateButton({
    ...
    Callback = function()
        ...
        -- Reset Visual Dropdowns (Pass empty table as 2nd arg)
        local newTargets = GetTargets()
        ...
        Rayfield:Notify({Title = "System", Content = "All Targets Cleared!", Duration = 3})
    end
})
...
OldTab:CreateButton({
    ...
    Callback = function()
        local newTargets = GetTargets()
        GojoDropdown:Refresh(newTargets, GojoTargetList) 
        RedDropdown:Refresh(newTargets, RedTargetList)
        CounterDropdown:Refresh(newTargets, CounterTargetList)
        PurpleDropdown:Refresh(newTargets, PurpleTargetList)
    end
})
