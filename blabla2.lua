...
local IsSummoningAction = false -- Variable baru untuk mengecek status summon
...
local function UseFold(duration)
    ...
    while os.clock() - start < duration do
        -- Fix: Hanya return jika semua mati, ATAU jika sedang summoning aktif
        if (not AutoGojo and not AutoRed and not AutoGojoRework) then return end
        if AutoKingMon and IsSummoningAction then return end 
...
local function ForceKillByVoid()
    ...
    while (AutoGojo or AutoRed or AutoGojoRework) and (os.clock() - startTime < 10) do 
        -- Stop void loop if summoning is active
        if AutoKingMon and IsSummoningAction then return end
...
Tab:CreateToggle({
    ...
    Flag = "AutoGojoRework", 
    Callback = function(v)
        -- REMOVED BLOCKING CHECK for AutoKingMon
        AutoGojoRework = v
        if v then
...
UtilityTab:CreateToggle({
    ...
    Callback = function(v)
        AutoKingMon = v
        if v then
            -- Note: We do NOT disable combat toggles automatically anymore.
            -- This allows combat to run when King Mon is present.
            
            -- But we do reset active follow states to prevent glitches during transition
            FollowTarget = false
            RedFollow = false
            HollowFollow = false
            CounterFollow = false
            ...
            Rayfield:Notify({Title = "System", Content = "Auto King Mon ON. Combat will pause ONLY when summoning.", Duration = 3})
        else
            IsSummoningAction = false -- Reset state
...
RunService.Heartbeat:Connect(function()
    -- Check: If AutoKingMon is ON AND actively summoning, disable combat movement.
    if AutoKingMon and IsSummoningAction then return end
...
task.spawn(function()
    while true do
        ...
        if Workspace:FindFirstChild("Living") and Workspace.Living:FindFirstChild("King Mon") then
            -- Boss exists: Disable Summoning Action flag so Combat can resume
            if IsSummoningAction then
                IsSummoningAction = false
                Rayfield:Notify({Title = "Battle", Content = "King Mon Alive! Combat Resumed.", Duration = 3})
            end
            -- Do NOT turn off AutoKingMon. Just wait here until he dies.
            task.wait(2)
            continue
        end

        -- Boss NOT exists: Enable Summoning Action flag to pause combat
        ...
        if nailCount > 0 then
            ...
            if spawnPart then
                -- PERBAIKAN DI SINI:
                -- Menggunakan FindFirstChild("ProximityPrompt") agar menargetkan nama yang persis.
                -- Ini akan menghindari "ProximityPromptB".
                ...
                if prompt then
                    ...
                    Rayfield:Notify({Title = "Error", Content = "Specific ProximityPrompt not found!", Duration = 3})
                end
            else
                Rayfield:Notify({Title = "Error", Content = "RuinedCity Spawn not found!", Duration = 3})
            end
            task.wait(3) -- Wait for summon
            continue 
        end
...
task.spawn(function()
    while true do
        task.wait(0.2)
        -- Check: Only pause if Summoning is ACTIVE
        if AutoKingMon and IsSummoningAction then continue end 
        ...
        if not CurrentGojoTarget then
            ...
            -- Only void if NOT summoning
            if not (AutoKingMon and IsSummoningAction) then
                ForceKillByVoid() 
            end
            ...
            repeat 
                task.wait(1) 
                CurrentGojoTarget = GetValidTargetFromList(GojoTargetList)
            until not AutoGojo or CurrentGojoTarget or (AutoKingMon and IsSummoningAction)
            task.wait(1)
            continue
        end
        ...
        while AutoGojo and not (AutoKingMon and IsSummoningAction) and hum.Health > 54 do
            RunService.Heartbeat:Wait() 
        end
        
        if not AutoGojo or (AutoKingMon and IsSummoningAction) then continue end
        ...
        while os.clock() - t < 52 do
            if not AutoGojo or (AutoKingMon and IsSummoningAction) then break end
            task.wait()
        end

        if AutoGojo and not (AutoKingMon and IsSummoningAction) then
            pcall(function()
    ...
    while true do
        ...
        if not CurrentRedTarget then
             ...
             repeat
                task.wait(0.1)
                CurrentRedTarget = GetValidTargetFromList(RedTargetList)
             until not AutoRed or CurrentRedTarget or (AutoKingMon and IsSummoningAction)
             
             if not AutoRed or (AutoKingMon and IsSummoningAction) then continue end
        end
        ...
        RedFollow = false
        if AutoRed and not (AutoKingMon and IsSummoningAction) then
            ForceKillByVoid()
        end
    end
end)
...
task.spawn(function()
    while task.wait(0.5) do
        -- Disable normal loot if summoning (as summoning has its own loot logic)
        if AutoKingMon and IsSummoningAction then 
            continue 
        end
        ...
        if #prompts > 0 then
            ...
            for _, prompt in ipairs(prompts) do
                if not AutoLoot or (AutoKingMon and IsSummoningAction) then break end
    ...
    while true do
        ...
        if not CurrentCounterTarget then
            ...
            repeat 
                task.wait(1)
                CurrentCounterTarget = GetValidTargetFromList(CounterTargetList)
            until not AutoGojoRework or CurrentCounterTarget or (AutoKingMon and IsSummoningAction)
        ...
        if not CurrentPurpleTarget then
             ...
             repeat
                task.wait(1)
                CurrentPurpleTarget = GetValidTargetFromList(PurpleTargetList)
             until not AutoGojoRework or CurrentPurpleTarget or (AutoKingMon and IsSummoningAction)
             
             if not AutoGojoRework or (AutoKingMon and IsSummoningAction) then continue end
        end
        ...
        while AutoGojoRework and not (AutoKingMon and IsSummoningAction) and hum.Health > 54 and IsTargetAlive(CurrentCounterTarget) do
        ...
        if CurrentCounterTarget then
            ...
            while (os.clock() - phase3Start < 12) and AutoGojoRework and not (AutoKingMon and IsSummoningAction) do
        ...
        while os.clock() - t < 50 do
            if not AutoGojoRework or (AutoKingMon and IsSummoningAction) then break end
            task.wait()
        end

        if AutoGojoRework and not (AutoKingMon and IsSummoningAction) then
            pcall(function()
                Gojo.HollowPurple:FireServer()
            end)
        end
