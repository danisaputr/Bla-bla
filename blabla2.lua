...
local Window = Rayfield:CreateWindow({
    ...
    LoadingSubtitle = "Updated Version",
...
task.spawn(function()
    while true do
        ...
        if AutoGojo then
            ...
            -- [PERBAIKAN] MENUNGGU RESPAWN BARU TP
            local newChar = LocalPlayer.CharacterAdded:Wait() -- Menunggu karakter baru
            local newHrp = newChar:WaitForChild("HumanoidRootPart", 5) 
            if newHrp then
                task.wait(0.2) -- Delay kecil agar fisika termuat
            ...
            if StandName.Value == "Standless" then
                ...
                -- [PERBAIKAN] MENUNGGU RESPAWN KEDUA (JIKA ADA)
                local newChar2 = LocalPlayer.CharacterAdded:Wait()
                local newHrp2 = newChar2:WaitForChild("HumanoidRootPart", 5)
                if newHrp2 then
                    task.wait(0.2)
                    newHrp2.CFrame = CFrame.new(-43, -116, 359)
                end
            end
        end
    end
