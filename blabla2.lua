...
local Window = Rayfield:CreateWindow({
    Name = "Auto Gojo + Auto Loot (Final Logic)",
    LoadingTitle = "System Loaded",
    LoadingSubtitle = "Hollow Priority Fix",
...
RunService.Heartbeat:Connect(function()
    ...
    -- === LOGIKA KHUSUS HOLLOW FOLLOW ===
    -- Jika HollowFollow aktif, PRIORITAS UTAMA adalah teleport ke CurrentGojoTarget
    if HollowFollow then
        if CurrentGojoTarget and CurrentGojoTarget:FindFirstChild("HumanoidRootPart") then
            local thrp = CurrentGojoTarget.HumanoidRootPart
            hrp.CFrame = GetHollowCFrame(thrp)
        end
        return -- Stop di sini agar logic lain tidak menimpa
    end

    -- === LOGIKA FOLLOW LAINNYA (Normal / Red) ===
    ...
    -- Hanya jalankan jika bukan HollowFollow (sudah dihandle di atas)
    if not (FollowTarget or RedFollow) then return end
...
task.spawn(function()
    while true do
        ...
        -- CATATAN: CurrentGojoTarget TIDAK diupdate ke dummy di sini.
        -- Jadi target tetap mengikuti pilihan Dropdown.
        ...
        while AutoGojoRework and hum.Health > 54 do
            ...
            -- Logic CFrame Internal khusus lawan Dummy (tanpa pake FollowTarget global)
        ...
        FollowTarget = false
        HollowFollow = true -- Memicu teleport ke CurrentGojoTarget (User Player)
