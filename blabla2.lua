...
local IsSavingToken = false -- Global Variable untuk status Saving Token
...
-- ===== AUTO CLEAR LOOP (FIXED LOGIC) =====
-- "Kondisi: jika saving token aktif maka jalankan, selebihnya pause jika bbq3 aktif"
task.spawn(function()
    while true do
        local CanExecute = false

        -- 1. Cek Toggle Utama dulu
        if AutoClear then
            -- 2. Cek apakah Auto BBQ3 Nyala?
            if AutoBBQ3 then
                -- Jika BBQ3 Nyala, kita cek STATUS Saving Token
                if IsSavingToken then
                    -- KONDISI 1: Saving Token Aktif (Farming) -> BOLEH CLEAR
                    CanExecute = true
                else
                    -- KONDISI 2: Saving Token Mati (Summon/Craft/Buy) -> JANGAN CLEAR
                    CanExecute = false
                end
            else
                -- KONDISI 3: Auto BBQ3 Mati -> Jalan normal sesuai Toggle AutoClear
                CanExecute = true
            end
        end
        ...
        if CanExecute then
            ...
            -- Timer Loop (Tunggu 2 menit atau sampai dimatikan)
            for i = 1, 120 do
                if not AutoClear then break end
                task.wait(1)
            end
        else
            -- Jika tidak boleh execute (Paused), cek lagi dalam 1 detik
            task.wait(1)
        end
    end
end)
...
task.spawn(function()
    while true do
        ...
        -- LOGIKA SAVING TOKEN (Mode Menabung)
        if ultraCount == 0 and meatCount < 5 then
            if IsSavingToken then
                if tokenValue >= 5000 then
                    IsSavingToken = false -- Target tercapai, matikan mode saving. Auto Clear akan Pause.
                    Rayfield:Notify({Title = "BBQ3", Content = "Token 5000 Terkumpul! Melanjutkan...", Duration = 3})
                else
                    IsSummoningAction = false -- Combat ON untuk farming
                    -- IsSavingToken = TRUE, Auto Clear akan JALAN.
                    Rayfield:Notify({Title = "Saving Token", Content = "Farming... (" .. tostring(tokenValue) .. "/5000)", Duration = 1})
                    task.wait(3)
                    continue
                end
            elseif tokenValue < 1000 then
                IsSavingToken = true -- Mulai Saving. Auto Clear akan JALAN.
                IsSummoningAction = false 
        ...
        -- ACTION PHASE (Summon / Craft / Buy)
        -- Di sini IsSavingToken = false. Maka Auto Clear akan PAUSE.
        IsSummoningAction = true
