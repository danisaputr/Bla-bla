...
local Window = Rayfield:CreateWindow({
    Name = "Auto Gojo (Rework Split Targets)",
    LoadingTitle = "System Loaded",
    LoadingSubtitle = "Specific Targeting",
...
-- COUNTER DUMMY DROPDOWN (REWORK PUNCH)
local CounterDropdown = Tab:CreateDropdown({
...
local PurpleDropdown = Tab:CreateDropdown({
    Name = "[Rework] Purple Targets (Ulti Phase)",
    Options = GetTargets(),
    CurrentOption = {},
    MultipleOptions = true,
    Multi = true,
...
Tab:CreateButton({
    ...
    Callback = function()
        ...
        CounterDropdown:Refresh(newTargets, true)
        PurpleDropdown:Refresh(newTargets, true)
    end
})
...
Tab:CreateToggle({
    Name = "Auto Gojo REWORK (Separate Targets)",
    CurrentValue = false,
    Callback = function(v)
        AutoGojoRework = v
        if v then
...
RunService.Heartbeat:Connect(function()
    ...
    if CounterFollow then
        ...
        if target and target:FindFirstChild("HumanoidRootPart") then
            ...
            -- Teleport depan target jarak 3, lock posisi (LookVector * 3)
            -- Kita pakai LookVector dari target agar kita berada di "depan" wajahnya
    ...
    elseif PurpleFollow then
        ...
        if target and target:FindFirstChild("HumanoidRootPart") then
            local thrp = target.HumanoidRootPart
            -- Copy HollowFollow: Lock posisi di atas target (26 studs)
...
task.spawn(function()
    while true do
        ...
        -- Step 1: Validasi Target Counter
        CurrentCounterTarget = GetValidTargetFromList(CounterTargetList)
        ...
        CounterFollow = true -- GANTI FollowTarget JADI CounterFollow
        task.wait(0.5)
        ...
        while (os.clock() - blabla < 10) and AutoGojoRework do
        ...
        -- Cek Target Purple khusus
        CurrentPurpleTarget = GetValidTargetFromList(PurpleTargetList)
        if not CurrentPurpleTarget then 
            -- Jika tidak ada target purple, pakai target counter sbg fallback
            CurrentPurpleTarget = CurrentCounterTarget 
        end

        PurpleFollow = true -- GANTI HollowFollow JADI PurpleFollow (Copy HollowFollow)
