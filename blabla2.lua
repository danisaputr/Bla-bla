...
-- ===== OPTIMIZED NETWORK VARIABLES (BYPASS WARP) =====
-- Path: ReplicatedStorage -> ABC... -> Utility -> Modules -> Warp -> Index -> Event -> Reliable
local WarpPath = Rep:WaitForChild("ABC - First Priority"):WaitForChild("Utility"):WaitForChild("Modules"):WaitForChild("Warp"):WaitForChild("Index")
local WarpEvent = WarpPath:WaitForChild("Event"):WaitForChild("Reliable")

-- Fungsi Helper untuk menembak langsung ke server (Mencegah lag akibat pembuatan table berulang)
local function FireDirectWarp(packetStr)
    pcall(function()
        WarpEvent:FireServer(
            buffer.fromstring("\018"), -- Arg 1 (Biasanya ID/Timestamp)
            buffer.fromstring(packetStr) -- Arg 2 (Payload Data)
        )
    end)
end
...
local AutoClearExp = false
local ExpWhitelist = {}
local AutoBPExchange = false -- [NEW VARIABLE: BP EXCHANGE]
...
local Window = Rayfield:CreateWindow({
    Name = "Auto Gojo [Rework Fixed]",
    LoadingTitle = "System Loaded",
    LoadingSubtitle = "BlaBla Fixed",
    ConfigurationSaving = {
        Enabled = true,
...
-- [NEW] AUTO CLEAR FILTERS
UtilityTab:CreateSection("Auto Clear Filters")

local WhitelistDropdown = UtilityTab:CreateDropdown({
    Name = "Whitelist Items (JANGAN HAPUS)",
...
UtilityTab:CreateButton({
    ...
    Callback = function()
        if WhitelistDropdown then 
            WhitelistDropdown:Refresh(getItemNameList(), ExpWhitelist) 
        end
    end
})

UtilityTab:CreateToggle({
    Name = "Auto Clear Exp (Filtered)",
    CurrentValue = false,
    Flag = "AutoClearExp", 
    Callback = function(v)
        AutoClearExp = v
        if v then
            Rayfield:Notify({Title = "System", Content = "Auto Clear Exp ON (Filter Mode)", Duration = 3})
        end
    end
})

UtilityTab:CreateToggle({
    Name = "Auto Clear All (Global - NO FILTER)",
    CurrentValue = false,
    Flag = "AutoClear", 
    Callback = function(v)
        AutoClear = v
        if v then
            Rayfield:Notify({Title = "System", Content = "WARNING: Auto Clear All menghapus SEMUA item!", Duration = 3})
        end
    end
})

-- [NEW] AUTO BP EXCHANGE TOGGLE
UtilityTab:CreateToggle({
...
UtilityTab:CreateToggle({
    ...
    Callback = function(v)
        ...
        if v then
            -- Reset other combats
            if AutoBBQ3 then AutoBBQ3 = false end -- Prioritize one boss
            FollowTarget = false
            RedFollow = false
            HollowFollow = false
            CounterFollow = false
...
UtilityTab:CreateToggle({
    ...
    Callback = function(v)
        AutoBBQ3 = v
        if v then
            if AutoKingMon then AutoKingMon = false end -- Prioritize one boss
            FollowTarget = false
            RedFollow = false
            HollowFollow = false
            CounterFollow = false
...
RunService.Heartbeat:Connect(function()
    if IsSummoningAction then return end -- Combined Block for KingMon/BBQ
...
-- ===== [NEW] AUTO CLEAR EXP LOGIC (WHITELIST SYSTEM) =====
-- Ini menggunakan client-side destroy untuk filter item spesifik
task.spawn(function()
            ...
            if backpack then
                for _, item in ipairs(backpack:GetChildren()) do
                    if item:IsA("Tool") then
                        ...
                        -- Cek apakah item ada di Whitelist
                        for _, safeName in ipairs(ExpWhitelist) do
                            if item.Name == safeName then
                                isSafe = true
                                break
                            end
                        end
                        
                        -- Cek juga apakah item sedang dipakai (Character) agar tidak terhapus saat dipegang
                        if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild(item.Name) then
                            isSafe = true
                        end

                        -- Jika TIDAK aman, musnahkan
                        if not isSafe then
                            item:Destroy()
                        end
                    end
...
-- ===== [NEW] AUTO CLEAR LOOP (DIRECT OPTIMIZED) =====
-- Ini menggunakan Remote Server untuk menghapus segalanya sekaligus
task.spawn(function()
    -- String Buffer untuk perintah "Clear" (Disiapkan di luar loop agar hemat memori)
    local clearPacket = "\254\001\000\006\005Clear"

    while true do
        if AutoClear then
            FireDirectWarp(clearPacket)
            -- Tunggu agak lama karena ini menghapus semua
            for i = 1, 60 do 
...
-- ===== [NEW] AUTO BP EXCHANGE LOOP =====
task.spawn(function()
    while true do
        task.wait(1.5) -- Cek setiap 1.5 detik agar tidak spamming terlalu parah
    ...
    while true do
        ...
        if not HasQuest110 then
            if soulCount < 10 then
                -- Farming logic omitted for brevity, same as original
...
-- ===== [NEW] AUTO BBQ3 SUMMON LOGIC (WITH COMBAT PAUSE) =====
task.spawn(function()
    local IsSavingToken = false -- Status apakah sedang menabung token
    ...
    while true do
        ...
        if not AutoBBQ3 then 
            IsSavingToken = false
            -- Jika King Mon juga mati, baru kita release combat
        ...
        -- 2. Cek Keberadaan Boss
        -- Jika Boss ada, MATIKAN mode summon (IsSummoningAction = false) agar AutoGojo bisa menyerang
        ...
        -- 3. LOGIKA SAVING TOKEN (Mode Menabung)
        -- Jika sedang menabung atau token sekarat saat butuh beli, Combat harus AKTIF (IsSummoningAction = false)
        if ultraCount == 0 and meatCount < 5 then
            -- A. Sedang dalam mode saving
            if IsSavingToken then
                if tokenValue >= 5000 then
                    IsSavingToken = false -- Target tercapai, lanjut eksekusi
                    Rayfield:Notify({Title = "BBQ3", Content = "Token 5000 Terkumpul! Melanjutkan...", Duration = 3})
                else
                    IsSummoningAction = false -- PENTING: Combat ON supaya bisa farming token
                    Rayfield:Notify({Title = "Saving Token", Content = "Farming... (" .. tostring(tokenValue) .. "/5000)", Duration = 1})
                    task.wait(3)
                    continue
                end
            
            -- B. Token sekarat (< 1000) -> Mulai mode saving
        ...
        -- 4. ACTION PHASE (Summon / Craft / Buy)
        -- Di fase ini, kita PAUSE COMBAT (IsSummoningAction = true) agar script fokus
        IsSummoningAction = true

        -- [A] SUMMON (Prioritas Utama)
        ...
        -- [C] BUY (Prioritas Terakhir - Hanya jika Token Aman)
        if ultraCount == 0 and meatCount < 5 then
...
task.spawn(function()
    while task.wait(0.5) do
        -- Disable if Summoning is happening
        if IsSummoningAction then 
            continue 
        end
