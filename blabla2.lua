...
-- =======================================================
-- ===== AUTO KING MON LOGIC (NEW FEATURE) ===============
-- =======================================================
task.spawn(function()
    while true do
        task.wait(1)
        ...
        -- 1. Check if King Mon already exists
        if Workspace:FindFirstChild("Living") and Workspace.Living:FindFirstChild("King Mon") then
            Rayfield:Notify({Title = "Success", Content = "King Mon Spawned Successfully!", Duration = 5})
            -- Stop Feature
            AutoKingMon = false
            -- Update UI Toggle if possible (visual only, logic is controlled by var)
            continue
        end
        ...
        -- 2. Jika TIDAK punya Quest 110
        if not HasQuest110 then
            local flameSouls = GetItemCount("Soul of Herrscher of Flamescion")
            
            -- Cek apakah butuh farming item
            if flameSouls < 10 then
                -- FARMING ITEM
                local foundItem = false
                if Workspace:FindFirstChild("Item") then
                    for _, drop in ipairs(Workspace.Item:GetChildren()) do
                        -- Check structure: Item -> ItemDrop (Model/Part) -> ItemName (Value)
                        -- Structure might vary, checking safest path based on request
                        local itemDrop = drop:FindFirstChild("ItemDrop")
                        if itemDrop then
                            local nameVal = drop:FindFirstChild("ItemName") -- Sometimes directly in Item
                            if not nameVal then nameVal = itemDrop:FindFirstChild("ItemName") end -- Or in ItemDrop
                            
                            -- Fallback: check drop Name itself or children
                            ...
                            if isTargetItem then
                                ...
                                if targetPart and prompt then
                                    ...
                                    foundItem = true
                                    break -- Ambil satu per satu lalu loop ulang cek inventory
                                end
                            end
                        end
                    end
                ...
                if not foundItem then
                    -- Rayfield:Notify({Title = "System", Content = "Looking for Soul of Herrscher...", Duration = 1})
                end
                task.wait(0.5) -- Delay antar item

            else 
                -- SUDAH PUNYA 10 ITEM -> BELI PANDORA & ACCEPT QUEST
                local hasPandora = GetItemCount("Pandora's Box") > 0
                
                if not hasPandora then
                    -- Beli Pandora
                    local args = {
            ...
            -- 3. JIKA SUDAH PUNYA QUEST 110
            
            -- Cek dulu apakah sudah punya Holy Nail
            local hasNail = GetItemCount("Holy Nail of Helena") > 0

            if not hasNail then
                -- Jalankan TurnIn 10 kali
                for i = 1, 10 do
                    if not AutoKingMon then break end
                    local args = {
                ...
                -- Teleport ke Anderson & Tembak Prompt
                local map = Workspace:FindFirstChild("Map")
                ...
                -- SUDAH PUNYA HOLY NAIL -> KE RUINED CITY SPAWN
                local map = Workspace:FindFirstChild("Map")
                ...
                if spawnPart then
                    ...
                    -- Tunggu sebentar agar tidak spam tp
                    task.wait(2)
                else
