...
local Window = Rayfield:CreateWindow({
    Name = "Auto Gojo + Auto Loot (Hollow Follow Fixed)",
    LoadingTitle = "System Loaded",
    LoadingSubtitle = "Hollow now tracks like Red",
...
-- =======================================================
-- ===== FIXED FOLLOW SYSTEM (HOLLOW SYNCED WITH RED) =====
-- =======================================================
RunService.Heartbeat:Connect(function()
    ...
    if AutoGojoRework then
        -- Jika mode rework, prioritas ke Dummy
        activeTarget = Workspace.Living:FindFirstChild("Counter Dummy")
        -- Fallback ke target player jika Dummy hilang
    ...
    if HollowFollow or RedFollow then
        -- SAMA PERSIS SEPERTI RED: 26 studs di atas kepala target
        hrp.CFrame = CFrame.new(thrp.Position + Vector3.new(0, 26, 0), thrp.Position)
        
        -- Reset velocity agar smooth dan tidak jatuh
        hrp.Velocity = Vector3.zero 
        hrp.AssemblyLinearVelocity = Vector3.zero
    elseif FollowTarget then
        -- Mode follow biasa (depan/belakang)
...
task.spawn(function()
    while true do
        ...
        FollowTarget = false
        HollowFollow = true -- Memicu teleport (Target sekarang otomatis diarahkan ke Dummy via RunService)
